...
function execute(brief, feedback, round) {
  ...
  const playbook = brief.cong_thuc_phong;
  const b = { ...brief }; delete b.dau_vao_tu_phong_khac; delete b.ceo_feedback; delete b.model_boost; delete b.cong_thuc_phong;
  return `TASK: ${JSON.stringify(b, null, 1)}
${playbook ? `CÔNG THỨC PHÒNG (đã đúc kết từ bài từng đạt điểm cao — bám theo khung này để đạt chất lượng):\n${playbook}\n` : ''}${handoff.length ? `DỮ LIỆU CÁC PHÒNG KHÁC ĐÃ BÀN GIAO (BẮT BUỘC dùng, trích số liệu/nội dung liên quan thay vì tự bịa):\n${handoff.join('\n---\n')}\n` : ''}${feedback ? `ĐÂY LÀ VÒNG SỬA THỨ ${round}. Nhận xét từ trưởng phòng vòng trước (bắt buộc sửa hết):\n${feedback}` : ''}
